...
(function ($) {
  ...
  $.extend($.fn.editableContainer.Popup.prototype, {
    ...
    show(closeAll) {
      ...
      let $label   = $('<label>').text(this.options.title || this.$element.data('title') || this.$element.data('originalTitle')),
  ...
  if ($.Poshytip) {  // need this check, because in inline mode poshytip may not be loaded!
    ...
    $.Poshytip.prototype.refresh = function (async) {
      ...
      let bgImage = this.$tip.css('background-image').match(reBgImage),
          arrow   = this.$arrow.css('background-image').match(reBgImage);

      if (bgImage) {
        let bgImagePNG = rePNG.test(bgImage[1]);
      ...
      const $table = this.$tip.find('table.fallback');
      if (ie6) {
        // fix min/max-width in IE6
        this.$tip[0].style.width = '';
        $table.width('auto').find('td').eq(3).width('auto');
        let tipW = this.$tip.width(),
...
(function ($) {
  ...
  $.extend(DateUI.prototype, {
    ...
    value2html(value, element) {
      const text = $.datepicker.formatDate(this.options.viewformat, value);
