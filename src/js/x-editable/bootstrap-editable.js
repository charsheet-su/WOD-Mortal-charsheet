...
(function ($) {
  ...
  const EditableForm = function (div, options) {
  ...
  EditableForm.prototype = {
    ...
    error(msg) {
      let $group = this.$form.find('.control-group'),
    ...
    submit(e) {
      ...
      let newValue = this.input.input2value();
      ...
      const submitValue = this.input.value2submit(newValue);
      ...
      $.when(this.save(submitValue))
        .done($.proxy(function (response) {
          ...
          const res = typeof this.options.success === 'function' ? this.options.success.call(this.options.scope, response, newValue) : null;
    ...
    save(submitValue) {
      ...
      let pk   = (typeof this.options.pk === 'function') ? this.options.pk.call(this.options.scope) : this.options.pk,
          /*
...
(function ($) {
  ...
  $.fn.editableutils = {
     ...
     */
    sliceObj(obj, keys, caseSensitive /* default: false */) {
      let key,
          keyLower,
          newObj = {};
     ...
     */
    getConfigData($element) {
      const data = {};
      $.each($element.data(), (k, v) => {
     ...
     */
    itemsByValue(value, sourceData, valueProp) {
      if (!sourceData || value === null) {
        return [];
      }

      if (typeof (valueProp) !== 'function') {
        const idKey = valueProp || 'value';
        valueProp = function (e) {
          return e[idKey];
        };
      }

      let isValArray = $.isArray(value),
          result     = [],
          that       = this;
      ...
      $.each(sourceData, (i, o) => {
        if (o.children) {
          ...
          /* jslint eqeq: true*/
          if (isValArray) {
            if ($.grep(value, v => v == (o && typeof o === 'object' ? valueProp(o) : o)).length) {
              result.push(o);
            }
          } else {
            const itemValue = (o && (typeof o === 'object')) ? valueProp(o) : o;
            if (value == itemValue) {
              result.push(o);
            }
          }
...
(function ($) {
  ...
  const Inline = function (element, options) {
    this.init(element, options);
  };
...
(function ($) {
  ...
  const Editable = function (element, options) {
    this.$element = $(element);
  ...
  Editable.prototype = {
    ...
    show(closeAll) {
      ...
      if (!this.container) {
        const containerOptions = $.extend({}, this.options, {
          value: this.value,
    ...
    switch (option) {
      ...
      case 'submit':  // collects value, validate and submit to server for creating new record
        ...
        if ($.isEmptyObject(errors)) {
          ...
          if ($elems.length === 1) {
            const editable = $elems.data('editable');
            // standard params
            let params = {
            ...
            const values = this.editable('getValue');
...
(function ($) {
  ...
  $.extend(List.prototype, {
    ...
    value2html(value, element, display, response) {
...
(function ($) {
  ...
  $.extend(Checklist.prototype, {
    ...
    input2value() {
      const checked = [];
      this.$input.filter(':checked').each((i, el) => {
        checked.push($(el).val());
      });
      return checked;
    },
    ...
    value2htmlFinal(value, element) {
      ...
      if (checked.length) {
        $.each(checked, (i, v) => {
          const text = escape ? $.fn.editableutils.escape(v.text) : v.text;
          html.push(text);
        });
  ...
  $.extend(Constructor.prototype, {
    ...
    value2input(value) {
      ...
      if (this.isRemote && !this.isMultiple && !this.options.select2.initSelection) {
        ...
        let customId   = this.options.select2.id,
            customText = this.options.select2.formatSelection;

        if (!customId && !customText) {
          const $el = $(this.options.scope);
          if (!$el.data('editable').isEmpty) {
            const data = {id: value, text: $el.text()};
...
(function ($) {
  ...
  const Combodate = function (element, options) {
    this.$element = $(element);
  ...
  Combodate.prototype = {
    ...
    getTemplate() {
      ...
      $.each(this.map, (k, v) => {
        v = v[0];
        const token = v.length > 1 ? v.substring(1, 2) : v;
    ...
    fillCombo(k) {
      ...
      const f = `fill${k.charAt(0).toUpperCase()}${k.slice(1)}`;
      const items = this[f]();
      const value = $combo.val();
    ...
    fillCommon(key) {
      ...
      if (this.options.firstItem === 'name') {
        ...
        let header = typeof relTime[key] === 'function' ? relTime[key](1, true, key, false) : relTime[key];
    ...
    fillDay() {
      ...
      if (this.options.smartDays && this.$month && this.$year) {
        const month = parseInt(this.$month.val(), 10);
        const year = parseInt(this.$year.val(), 10);
     ...
     */
    fillAmpm() {
      let ampmL = this.options.template.indexOf('a') !== -1,
    ...
    setValue(value) {
      ...
      let dt     = typeof value === 'string' ? moment(value, this.options.format) : moment(value),
          that   = this,
          values = {};
      ...
      function getNearest($select, value) {
        let delta = {};
        $select.children('option').each((i, opt) => {
          let optValue = $(opt).attr('value'),
              distance;
    ...
    highlight(dt) {
      ...
      } else if (this.options.errorClass) {
...
(function ($) {
  ...
  $.extend(Constructor.prototype, {
    ...
    value2html(value, element) {
      const text = value ? value.format(this.options.viewformat) : '';
      // $(element).text(text);
      Constructor.superclass.value2html.call(this, text, element);
    },
...
(function ($) {
  ...
  Datepicker.prototype = {
    ...
    _trigger(event, altdate) {
      ...
      this.element.trigger({
        ...
        format: $.proxy(function (altformat) {
          const format = altformat || this.o.format;
    ...
    place() {
      ...
      const height = this.component ? this.component.outerHeight(true) : this.element.outerHeight(true);
      this.picker.css({
    ...
    fill() {
      ...
      while (prevMonth.valueOf() < nextMonth) {
        ...
        let before = this.o.beforeShowDay(prevMonth);
        if (before === undefined) {
          before = {};
  ...
  DateRangePicker.prototype = {
    updateDates() {
      this.dates = $.map(this.pickers, i => i.date);
      this.updateRanges();
    },
    updateRanges() {
  ...
  function opts_from_el(el, prefix) {
    ...
    for (const key in data) {
      if (prefix.test(key)) {
        inkey = key.replace(replace, (_, a) => a.toLowerCase());
        out[inkey] = data[key];
      }
    }
    return out;
...
(function ($) {
  ...
  $.extend(Date.prototype, {
    ...
    value2html(value, element) {
      const text = value ? this.dpg.formatDate(value, this.parsedViewFormat, this.options.datepicker.language) : '';
  ...
  $.extend(DateTime.prototype, {
    ...
    render() {
      ...
      this.$input.on('changeMode', function (e) {
        const f = $(this).closest('form').parent();
    ...
    value2html(value, element) {
      ...
      const text = value ? this.dpg.formatDate(this.toUTC(value), this.parsedViewFormat, this.options.datetimepicker.language, this.options.formatType) : '';
      if (element) {
    ...
    html2value(html) {
      ...
      const value = this.parseDate(html, this.parsedViewFormat);
    ...
    str2value(str) {
      // parseDate return utc date!
      const value = this.parseDate(str, this.parsedFormat);
      return value ? this.fromUTC(value) : null;
    },
    ...
    input2value() {
      ...
      const dt = this.$input.data('datetimepicker');
