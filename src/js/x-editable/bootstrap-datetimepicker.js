...
!(function ($) {
  ...
  Datetimepicker.prototype = {
    ...
    fill() {
      ...
      if (this.formatViewType == 'time') {
        const hourConverted = hours % 12 ? hours % 12 : 12;
        const hoursDisplay = (hourConverted < 10 ? '0' : '') + hourConverted;
        const minutesDisplay = (minutes < 10 ? '0' : '') + minutes;
        const meridianDisplay = dates[this.language].meridiem[hours < 12 ? 0 : 1];
      ...
      const months = this.picker.find('.datetimepicker-months')
        .find('th:eq(1)')
        .text(year)
        .end()
      ...
      const yearCont = this.picker.find('.datetimepicker-years')
        .find('th:eq(1)')
        .text(`${year}-${year + 9}`)
        .end()
        .find('td');
    ...
    click(e) {
      e.stopPropagation();
      e.preventDefault();
      const target = $(e.target).closest('span, td, th, legend');
      if (target.length == 1) {
  ...
  var DPGlobal = {
    ...
    parseDate(date, format, language, type) {
    ...
    formatDate(date, format, language, type) {
      if (date == null) {
        return '';
      }
      let val;
